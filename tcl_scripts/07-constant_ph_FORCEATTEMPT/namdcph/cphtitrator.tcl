...
proc ::cphTitrator::proposeMove {pH} {
    ...
    set moveLabels [shuffle [cphTitrator get moveLabels]]
#    puts "Alex: Looping over all moveLabels (ignoring weights): $moveLabels"
    foreach moveLabel $moveLabels {
		set proposalCmd [cphTitrator get proposalCmd $moveLabel]
		set accept [eval $proposalCmd $pH 0]
		puts "Alex: Trying moveLabel: $moveLabel, accept: $accept"
	...
	puts "Alex: no acceptable pre-switch found, returning one at random"
	lassign [choice $moveLabels $weights] moveLabel
	set proposalCmd [cphTitrator get proposalCmd $moveLabel]
	set accept [eval $proposalCmd $pH 1] 
	puts "Alex: Trying moveLabel: $moveLabel, accept: $accept"
	set segresidnameList [cphTitrator get segresidnameList $moveLabel]
	set numsteps [cphTitrator get numsteps $moveLabel]
...
#
proc ::cphTitrator::proposeResidueMove {segresidname pH Force} {
    lassign [cphSystem compute inherentWeights $pH $segresidname] states weights
    ...
    puts "$segresidname du: $du"
    set accept [metropolisAcceptance $du]
	
	if { $Force == 1 } {
		puts "Alex is FORCING an acceptance in cphtitrator.tcl so a switch will be attempted"
		set accept 1
	}
...
proc ::cphTitrator::buildTitrator {moveInfo} {
    ...
    foreach segresidname [cphSystem get segresidnames] {
        puts "::cphTitrator::buildTitrator $segresidname"
