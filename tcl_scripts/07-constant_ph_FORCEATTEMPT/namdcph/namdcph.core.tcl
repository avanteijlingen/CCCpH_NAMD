...
proc call_python {basename cycle} {
    set output [exec python /users/rkb19187/Desktop/Constant_pH/files/07-constant_ph/namdcph/support.py $basename $cycle]
    puts $output
}
...
proc ::namdcph::cphRun {numsteps {numcycles 1}} {
	...
	variable ::namdcph::configConstantCharge

    # Initialize NAMD and build a constant pH enabled PSF. (namdcph.md.pdb/psf)
    initialize
    finalize
    ...
    #Do the initial charge neutralization
	if {$configConstantCharge} {
		# (0) check charges and add counter ions
		call_python [getMDBasename] 0
    ...
    for {set cycle $firstCycle} {$cycle <= $lastCycle} {incr cycle} { 
		...
		# (2.0) get list of all titratable groups
        #set segresidnameList [cphSystem get segresidnames]
		#puts "segresidnameList $segresidnameList"
		
		# (2) Propose a move from the full move set.
        lassign [cphTitrator propose $SystempH] accept swNumsteps segresidnameList
        ...
        #
		puts "namdcph.core.tcl (3) accept: $accept"
        if {!$accept} {
            ...
            puts "Alex: Running Switch $segresidnameList, accept: $accept"
            set runArgs [list $numsteps]
		...
		puts "namdcph.core.tcl (3) swRunSwitch accept: $accept"
#exit
		...
		if {$configConstantCharge} {
			# (0) check charges and add counter ions
			call_python [getMDBasename] $cycle
			#reloadAndReinit [getMDBasename] false
...
proc ::namdcph::ConstantCharge {CC} {
    variable ::namdcph::configConstantCharge
	if {$CC == "on" || $CC == "yes"} {
		puts "Constant Charge: on"
		set configConstantCharge True
	} else {
		puts "Constant Charge: off"
		set configConstantCharge False
	}
    return
}
...
proc ::namdcph::runSwitch {numsteps segresidnameList} {
	...
	puts "DeltaE: $DeltaE for $segresidnameList useHeat: $useHeat Work: $Work ReducedWork: $ReducedWork"
...
proc ::namdcph::dealchemify {segresidnameList} {
    ...
    set gMDB getMDBasename
puts "getMDBasename: $gMDB"
    alch off
    reloadAndReinit [getMDBasename] false
    return
}
